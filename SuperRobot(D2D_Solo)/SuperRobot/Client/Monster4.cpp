...
HRESULT CMonster4::Ready_GameObject()
{
	m_tMonInfo.bTurn = false;
	m_tMonInfo.vPos = _vec3(600.f, 680.f, 0.f);
	...
	m_pTextureMonster = L"Monster03";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Monster", L"Monster03", 0);


	lstrcpy(m_tMonInfo.szUnitName, L"엑스트라");
	lstrcpy(m_tMonInfo.szName, L"이름없..");
	lstrcpy(m_tMonInfo.szToking, L"「난 주연이 될거야」");
	...
	m_tMonInfo.iGold = 675;
	return S_OK;
}


int CMonster4::Update_GameObject()
{
	if (m_bDead)
	{
...
void CMonster4::LateUpdate_GameObject()
{
...
void CMonster4::Render_GameObject()
{
...
void CMonster4::Release_GameObject()
{
}
