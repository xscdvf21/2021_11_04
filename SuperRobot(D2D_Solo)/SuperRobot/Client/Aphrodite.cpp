...
HRESULT CAphrodite::Ready_GameObject()
{
	m_tInfo.vPos = { 140.f, 140.f, 0.f };
	...
	m_pTextureUnit = L"Aphrodite";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Unit", L"Aphrodite", 0);


	lstrcpy(m_tInfo.szUnitName, L"아프로");
	lstrcpy(m_tInfo.szName, L"112기 부반장");
	lstrcpy(m_tInfo.szToking, L"「이름짓기가 넘 힘들어요」");
	lstrcpy(m_tInfo.szToking2, L"「선생님 나빠요...」");
	lstrcpy(m_tInfo.szWeaponName, L"아프로 링");
	m_tInfo.iLevel = 3;
	m_tInfo.iHp = 2500;
	m_tInfo.iMaxHp = 2500;
	m_tInfo.iMove = 2; //캐릭 이동력.
	m_tInfo.iArmor = 150;
	m_tInfo.iEnergy = 100;
...
int CAphrodite::Update_GameObject()
{
	if (m_bDead)
	{
...
void CAphrodite::LateUpdate_GameObject()
{
}

void CAphrodite::Render_GameObject()
{
	...
	if (nullptr == pTexInfo)
	{
		ERR_MSG(L"Aphrodite.Cpp, pTexInfo가 nullptr");
		return;
	}
...
void CAphrodite::Release_GameObject()
{
}

void CAphrodite::Key_Check() //움직이는거 체크
{
...
void CAphrodite::Attack_Check() //이동 한 후 공격 할 유닛 찾는.
{
