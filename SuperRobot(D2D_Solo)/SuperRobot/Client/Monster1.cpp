...
HRESULT CMonster1::Ready_GameObject()
{
	m_tMonInfo.bTurn = false;
	m_tMonInfo.vPos = _vec3(400.f, 600.f, 0.f);
	...
	m_pTextureMonster = L"Monster00";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Monster", L"Monster00", 0);


	lstrcpy(m_tMonInfo.szUnitName, L"병건쌤로봇");
	lstrcpy(m_tMonInfo.szName, L"박병건쌤");
	lstrcpy(m_tMonInfo.szToking, L"「다음달에는 더 바쁠꺼야 ^^」");
	lstrcpy(m_tMonInfo.szToking2, L"「?? 무슨소리야 나는 아닌데 ?」");
	m_tMonInfo.iLevel = 2;
	m_tMonInfo.iHp = 1500;
	m_tMonInfo.iMaxHp = 1500;
	m_tMonInfo.iMove = 3; //캐릭 이동력.
	m_tMonInfo.iAttack = 900;
	m_tMonInfo.iArmor = 250;
	m_tMonInfo.iEnergy = 160;
	m_tMonInfo.iGold = 400;
...
int CMonster1::Update_GameObject()
{
	if (m_bDead)
	{
...
void CMonster1::LateUpdate_GameObject()
{
...
void CMonster1::Render_GameObject()
{
	...
	if (nullptr == pTexInfo)
	{
		ERR_MSG(L"Monster1.Cpp, pTexInfo가 nullptr");
		return;
	}
...
void CMonster1::Release_GameObject()
{
}
...
//bool CMonster1::compare(pair<float, TILE> _a, pair<float, TILE> _b)
//{
//	if (_a.first == _b.first)
//		return _a.second.iHp < _b.second.iHp;
//
