...
HRESULT CGetta::Ready_GameObject()
{
	m_tInfo.vPos = { 140.f, 60.f, 0.f };
	...
	m_pTextureUnit = L"Getta";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Unit", L"Getta", 0);


	lstrcpy(m_tInfo.szUnitName, L"겟타");
	lstrcpy(m_tInfo.szName, L"정달영");
	lstrcpy(m_tInfo.szToking, L"「강력한 빔이다.」");
	lstrcpy(m_tInfo.szToking2, L"「아파요.. 아파..」");
	lstrcpy(m_tInfo.szWeaponName, L"겟타 빔");
	m_tInfo.iLevel = 4;
	m_tInfo.iHp = 4000;
	m_tInfo.iMaxHp = 4000;
	m_tInfo.iMove = 3; //캐릭 이동력.
	m_tInfo.iArmor = 130;
	m_tInfo.iEnergy = 100;
...
void CGetta::LateUpdate_GameObject()
{

}

void CGetta::Render_GameObject()
{
	...
	if (nullptr == pTexInfo)
	{
		ERR_MSG(L"CGetta.Cpp, pTexInfo가 nullptr");
		return;
	}
...
void CGetta::Release_GameObject()
{
}

void CGetta::Key_Check() //움직이는거 체크
{
...
void CGetta::Attack_Check() //이동 한 후 공격 할 유닛 찾는.
{
