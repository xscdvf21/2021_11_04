...
HRESULT CMazingerZ::Ready_GameObject()
{
	m_tInfo.vPos = { 60.f, 140.f, 0.f };
	...
	m_pTextureUnit = L"MazingerZ";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Unit", L"MazingerZ", 0);


	lstrcpy(m_tInfo.szUnitName, L"마징가Z");
	lstrcpy(m_tInfo.szName, L"코우지");
	lstrcpy(m_tInfo.szToking, L"「시연회 준비로 시체가 되고있어..」");
	lstrcpy(m_tInfo.szToking2, L"「나는 괜찮아..할 수 있어.」");
	lstrcpy(m_tInfo.szWeaponName, L"마징가 빔");
	m_tInfo.iLevel = 7;
	m_tInfo.iHp = 4000;
	m_tInfo.iMaxHp = 4000;
	m_tInfo.iMove = 5; //캐릭 이동력.
	m_tInfo.iArmor = 250;
	m_tInfo.iEnergy = 150;
	m_tInfo.iMaxEnergy = 150;
	m_tInfo.iAttack = 600;
	m_tInfo.iRange = 5;
...
int CMazingerZ::Update_GameObject()
{
	if (m_bDead)
	{
...
void CMazingerZ::LateUpdate_GameObject()
{
}

void CMazingerZ::Render_GameObject()
{
...
void CMazingerZ::Release_GameObject()
{
}


void CMazingerZ::Key_Check() //움직이는거 체크
{
...
void CMazingerZ::Attack_Check() //이동 한 후 공격 할 유닛 찾는.
{
...
void CMazingerZ::Move_Route()
{
	...
	if (fDist < 3.f)
	{
		...
		if (BestList.empty())
		{
			CGameObject_Manger::Get_Instance()->Get_Tile()->Set_MoveTileCheck(false);
			bAStarCheck = true;
		}
	}
