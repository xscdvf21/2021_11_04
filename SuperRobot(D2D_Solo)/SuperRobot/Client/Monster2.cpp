...
HRESULT CMonster2::Ready_GameObject()
{
	m_tMonInfo.vPos = _vec3(640.f, 640.f, 0.f);
	m_tMonInfo.vSize = _vec3(1.0f, 1.0f, 0.f);
	...
	m_pTextureMonster = L"Monster01";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Monster", L"Monster01", 0);

	lstrcpy(m_tMonInfo.szUnitName, L"우재쌤로봇");
	lstrcpy(m_tMonInfo.szName, L"신우재쌤	");
	lstrcpy(m_tMonInfo.szToking, L"「시연회가 힘들어?? 왜? 더 노력해.」");
	lstrcpy(m_tMonInfo.szToking2, L"「이상한 소리 하지마.」");
	m_tMonInfo.iLevel = 4;
	m_tMonInfo.iHp = 1200;
	m_tMonInfo.iMaxHp = 1200;
	m_tMonInfo.iMove = 3; //캐릭 이동력.
	m_tMonInfo.iAttack = 800;
	m_tMonInfo.iArmor = 110;
	m_tMonInfo.iEnergy = 160;
	m_tMonInfo.iGold = 700;
	return S_OK;
}

int CMonster2::Update_GameObject()
{
	if (m_bDead)
	{
...
void CMonster2::LateUpdate_GameObject()
{
...
void CMonster2::Render_GameObject()
{
...
void CMonster2::Release_GameObject()
{
}

void CMonster2::Move_Check()
{
