...
HRESULT CMonster05::Ready_GameObject()
{
	m_tMonInfo.bTurn = false;
	m_tMonInfo.vPos = _vec3(560.f, 560.f, 0.f);
	...
	m_pTextureMonster = L"Monster04";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Monster", L"Monster04", 0);


	lstrcpy(m_tMonInfo.szUnitName, L"만든시간");
	lstrcpy(m_tMonInfo.szName, L"01시23분");
	lstrcpy(m_tMonInfo.szToking, L"「다음달에는 더 바쁠꺼야 ^^」");
	lstrcpy(m_tMonInfo.szToking2, L"「?? 무슨소리야 나는 아닌데 ?」");
	m_tMonInfo.iLevel = 2;
	m_tMonInfo.iHp = 100;
	m_tMonInfo.iMaxHp = 3000;
	m_tMonInfo.iMove = 2; //캐릭 이동력.
	m_tMonInfo.iAttack = 450;
	m_tMonInfo.iArmor = 250;
	m_tMonInfo.iEnergy = 160;
	m_tMonInfo.iGold = 500;
	return S_OK;
}

int CMonster05::Update_GameObject()
{
	if (m_bDead)
	{
...
void CMonster05::LateUpdate_GameObject()
{
...
void CMonster05::Render_GameObject()
{
	...
	if (nullptr == pTexInfo)
	{
		ERR_MSG(L"Monster5.Cpp, pTexInfo가 nullptr");
		return;
	}
...
void CMonster05::Release_GameObject()
{
}

void CMonster05::Move_Check()
{
	//////////////////////
