...
HRESULT CWhitebase::Ready_GameObject()
{
	...
	m_pTextureUnit = L"Whitebase";
	pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_TextureMgr(L"Unit", L"Whitebase", 0);


	lstrcpy(m_tInfo.szUnitName, L"화이트");
	lstrcpy(m_tInfo.szName, L"112기 반장");
	lstrcpy(m_tInfo.szToking, L"「무슨말을 할까」");
	lstrcpy(m_tInfo.szToking2, L"「선생님 나빠요...」");
	lstrcpy(m_tInfo.szWeaponName, L"레이저 빔");
	m_tInfo.iLevel = 2;
	m_tInfo.iHp = 3400;
	m_tInfo.iMaxHp = 4000;
	...
	m_tInfo.iAttack = 1000;
	m_tInfo.iRange = 7;
	return S_OK;

}

int CWhitebase::Update_GameObject()
{
	if (m_bDead)
	{
...
void CWhitebase::LateUpdate_GameObject()
{
}

void CWhitebase::Render_GameObject()
{
	...
	if (nullptr == pTexInfo)
	{
		ERR_MSG(L"Whitebase.Cpp, pTexInfo가 nullptr");
		return;
	}
...
void CWhitebase::Release_GameObject()
{
}

void CWhitebase::Key_Check() //움직이는거 체크
{
...
void CWhitebase::Attack_Check() //이동 한 후 공격 할 유닛 찾는.
{
