...
void CFontMgr::Render_Font3D(const _tchar* pFontTag, const _tchar * pString, const _vec2* pPos, const _matrix _PlayerWorld, D3DXCOLOR Color)
{
	CFont*	pFont = Find_Font(pFontTag);
	NULL_CHECK(pFont);

	pFont->Render_Font3D(pString, pPos, _PlayerWorld, Color);
}
