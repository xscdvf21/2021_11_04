...
HRESULT CBossBullet::Ready_Object(void)
{
	...
	//pGameObject = CBulletWind::Create(m_pGraphicDev, m_vDir, 1.f);
...
HRESULT CBossBullet::Ready_Object(_vec3 vPos)
{
	FAILED_CHECK_RETURN(Add_Component(), E_FAIL);
	...
	pGameObject = CBulletWind::Create(m_pGraphicDev, m_vDir, vPos, 2.f);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"Effect_BulletWind", pGameObject), E_FAIL);

	pGameObject = CBulletWind::Create(m_pGraphicDev, m_vDir, vPos, 3.5f);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"Effect_BulletWind2", pGameObject), E_FAIL);

	pGameObject = CBulletWind::Create(m_pGraphicDev, m_vDir, vPos, 5.f);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
...
_int CBossBullet::Update_Object(const _float & fTimeDelta)
{
...
void CBossBullet::Render_Object(void)
{
...
HRESULT CBossBullet::Add_Component(void)
{
...
void CBossBullet::SetUp_Sphere(void)
{
...
_bool CBossBullet::Collision_ToObject(const _tchar * pLayerTag, const _tchar * pObjTag, const _float & fTimeDelta)
{
	CManagement*	pManagement = CManagement::GetInstance();
...
CBossBullet * CBossBullet::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CBossBullet*	pInstance = new CBossBullet(pGraphicDev);
