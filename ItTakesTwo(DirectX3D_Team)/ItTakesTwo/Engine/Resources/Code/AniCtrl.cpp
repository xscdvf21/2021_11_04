...
void Engine::CAniCtrl::Set_Animationset(const _uint& iIndex, _float fspeed, _float fSyncTime, _float fDelay)
{
	...
	m_pAniCtrl->KeyTrackEnable(m_iCurrentTrack, FALSE, m_fAccTime + fSyncTime);

	// 트랙이 해제되는 시간동안 현재 애니메이션 셋은 어떤 속도로 움직일지 결정(속도의 상수 값은 각자 1)
	m_pAniCtrl->KeyTrackSpeed(m_iCurrentTrack, fspeed, m_fAccTime, fSyncTime, D3DXTRANSITION_LINEAR);

	// 트랙이 해제되는 시간동안 현재 애니메이션 셋은 어떤 가중치를 갖게 할 지(속도의 상수 값은 각자 1)
	m_pAniCtrl->KeyTrackWeight(m_iCurrentTrack, 0.1f, m_fAccTime, fSyncTime, D3DXTRANSITION_LINEAR);


	// 트랙 활성화의 유무를 판단하는 함수
	m_pAniCtrl->SetTrackEnable(m_iNewTrack, TRUE);
	// 트랙이 시작하는 시간동안 현재 애니메이션 셋은 어떤 속도로 움직일지 결정(속도의 상수 값은 각자 1)
	m_pAniCtrl->KeyTrackSpeed(m_iNewTrack, fspeed, m_fAccTime, fSyncTime, D3DXTRANSITION_LINEAR);
	// 트랙이 시작하는 시간동안 현재 애니메이션 셋은 어떤 가중치를 갖게 할 지(속도의 상수 값은 각자 1)
	m_pAniCtrl->KeyTrackWeight(m_iNewTrack, 0.9f, m_fAccTime, fSyncTime, D3DXTRANSITION_LINEAR);


	m_pAniCtrl->ResetTime(); // 애니메이션이 재생되던 시간을 다시 세팅(advanced함수 호출 시 내부적으로 누적되던 시간 리셋)
	m_fAccTime = 0.f;
	
	m_pAniCtrl->SetTrackPosition(m_iNewTrack, fDelay);
...
Engine::_bool Engine::CAniCtrl::Is_AnimationSetEnd(_float minusTime, _bool bPosReset)
{
	D3DXTRACK_DESC		TrackInfo;
	ZeroMemory(&TrackInfo, sizeof(D3DXTRACK_DESC));
	...
	if (TrackInfo.Position >= m_dPeriod - minusTime)
	{
