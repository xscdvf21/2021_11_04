...
CCord::CCord(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CCord::Ready_Object(void* pArg)
{
...
Engine::_int CCord::Update_Object(const _float& fTimeDelta)
{
	if (m_bDead)
		return OBJ_DEAD;
	...
	for (_int i = 0; i < 30; ++i)
	{
		...
		tParticle2.vPos = m_pTransformCom->Get_Position() + _vec3(-0.7f, 1.2f, -1.f) + _vec3(CRandoms()(-0.5f, 0.5f), CRandoms()(-0.5f, 0.5f), CRandoms()(-0.5f, 0.5f));
	...
	tParticle.vPos = m_pTransformCom->Get_Position() + _vec3(-0.7f, 1.2f, -1.f);
...
_int CCord::LateUpdate_Object(const _float & fTimeDelta)
{
	_matrix matWorld;
...
void CCord::Render_Object(const _int& iIndex)
{
...
CCord* CCord::Create(LPDIRECT3DDEVICE9 pGraphicDev, void* pArg)
{
	CCord*	pInstance = new CCord(pGraphicDev);
...
void CCord::Free(void)
{
	Engine::CGameObject::Free();
}

HRESULT CCord::Add_Component(void* pArg)
{
	...
	// CStaticMesh
	pComponent = m_pMeshCom = dynamic_cast<Engine::CStaticMesh*>(Engine::Clone_Resource(tTemp.eSceneID, L"Cord"));
...
HRESULT CCord::SetUp_ConstantTable(LPD3DXEFFECT & pEffect, const _int & iIndex)
{
