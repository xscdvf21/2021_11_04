...
void CCody::Set_ActorSetting(PxRigidDynamic* pActor, PxShape* pBody)
{
...
physx::PxControllerBehaviorFlags CCody::getBehaviorFlags(const PxShape& shape, const PxActor& actor)
{
...
physx::PxControllerBehaviorFlags CCody::getBehaviorFlags(const PxController& controller)
{
...
physx::PxControllerBehaviorFlags CCody::getBehaviorFlags(const PxObstacle& obstacle)
{
...
void CCody::onShapeHit(const PxControllerShapeHit& hit)
{
	if (hit.shape->getSimulationFilterData().word0 & FilterGroup::eHoze)
	{
...
void CCody::onControllerHit(const PxControllersHit& hit)
{
	OutputDebugString(L"다른 컨트롤러랑 충돌\n");
	return;
}

void CCody::onObstacleHit(const PxControllerObstacleHit& hit)
{
	OutputDebugString(L"옵스타클이랑 충돌\n");
	return;
}

bool CCody::filter(const PxController& a, const PxController& b)
{
	PxShape* pShape1;
	PxShape* pShape2;
	pShape1 = nullptr;
...
physx::PxQueryHitType::Enum CCody::preFilter(const PxFilterData& filterData, const PxShape* shape, const PxRigidActor* actor, PxHitFlags& queryFlags)
{
	return PxQueryHitType::eBLOCK;

}

physx::PxQueryHitType::Enum CCody::postFilter(const PxFilterData& filterData, const PxQueryHit& hit)
{
	return PxQueryHitType::eBLOCK;
...
Engine::_bool CCody::Check_Insert()
{
	m_bInsert = false;
	return m_bInsert;
}

CCody::CCody(LPDIRECT3DDEVICE9 pGraphicDev)
...
void CCody::Check_Wall(const _float& fTimeDelta)
{
	const PxU32 bufferSize = 256;				// [in] size of 'hitBuffer'
	PxRaycastHit hitBuffer[bufferSize];			// [out] User provided buffer for results
	PxRaycastBuffer buf(hitBuffer, bufferSize); // [out] Blocking and touching hits stored here
		...
		buf, PxHitFlag::eDEFAULT, data))
	{
		...
		{
			if (buf.getTouch(i).shape->getSimulationFilterData().word0 == FilterGroup::eCody)
			{
				continue;
			}
			else
		...
		if (m_eCodyAnim == Cody_Wall_Idle)
			m_bClimbingFall = true;
	}
}

HRESULT CCody::Ready_Object(void* pArg)
{
...
Engine::_int CCody::Update_Object(const _float& fTimeDelta)
{
	...
	if (!m_bKeyOff)
	{
		...
		// 애니메이션
		//if (!m_bPlayerDead)
		//	PlayAnimation(this, m_pMeshCom, fTimeDelta * m_fAnimRatio, m_ePL_STATE, &m_ePre_State);
		Effect(fTimeDelta);
...
_int CCody::LateUpdate_Object(const _float & fTimeDelta)
{
	if (m_bKeyOff)
		return 0;
	...
	if (m_bStart)
	{
		if (m_tObjInfo.eSceneID == RESOURCE_VF2)
			StopNPlaySound(L"VF2_CodyStart.ogg", CSoundMgr::CHANNELID::CrusherHitPoint, 0.3f);
		Create_UI(L"Cody_UI");
		m_bStart = false;
	}
...
void CCody::Render_Object(const _int& iIndex)
{
...
void CCody::Render_Shadow(_vec3 * vEye, _vec3 * vAt, const _int & iIndex)
{
	LPD3DXEFFECT	pEffect = m_pShaderCom->Get_EffectHandle();
	NULL_CHECK(pEffect);
	pEffect->AddRef();
...
void CCody::Render_Stencil(const _int & iIndex)
{
	LPD3DXEFFECT	pEffect = m_pShaderCom->Get_EffectHandle();
	NULL_CHECK(pEffect);
	pEffect->AddRef();
...
void CCody::Render_VTF(const _int& iIndex /*= 0*/)
{
	LPD3DXEFFECT	pEffect = m_pShaderCom->Get_EffectHandle();
	NULL_CHECK(pEffect);
	pEffect->AddRef();
...
void CCody::Effect(const _float & fTimeDelta)
{
...
void CCody::Effect_Walk(const _float & fTimeDelta)
{
...
void CCody::Effect_Dead(const _float & fTimeDelta)
{
	if (!m_bDeadEffect)
		return;
...
void CCody::Effect_Respawn(const _float & fTimedelta)
{
	if (!m_bRespawnEffect)
		return;
...
void CCody::Effect_Hit(const _float & fTimeDelta)
{
	if (!m_bHitEffect)
		return;
...
void CCody::Effect_Dust(const _float & fTimeDelta)
{
	PARTICLE tParticle;
	Engine::Reset_Particle(tParticle);
...
CCody* CCody::Create(LPDIRECT3DDEVICE9 pGraphicDev, void* pArg)
{
	CCody*	pInstance = new CCody(pGraphicDev);
...
HRESULT CCody::Add_Component(void* pArg)
{
	...
	// Texture
	pComponent = m_pTextureCom = static_cast<Engine::CTexture*>(Engine::Clone_Resource(RESOURCE_STATIC, L"Dissolve"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	...
	Create_Actor(Engine::Get_Physics(), m_fShapeHeight, m_fShapeRadius, this, this, { tTemp.vPos.x, tTemp.vPos.y + m_fHalfHeight, tTemp.vPos.z }, { tTemp.vAngle.x,tTemp.vAngle.y,tTemp.vAngle.z });
...
void CCody::Key_Input(const _float& fTimeDelta)
{
	if (Lock())
		return;
	...
	if (Engine::Key_Down(DIK_CAPSLOCK))
		m_iKeyState |= RUN;
...
HRESULT CCody::SetUp_ConstantTable(LPD3DXEFFECT & pEffect, const _int& iIndex)
{
	...
	SetUp_OnRegister(0, L"Target_VTF_Pos_Cody");
	SetUp_OnRegister(1, L"Target_VTF_Normal_Cody");
	SetUp_OnRegister(2, L"Target_VTF_Tangent_Cody");
	SetUp_OnRegister(3, L"Target_VTF_Binormal_Cody");
...
void CCody::Set_State(const _float & fTimeDelta)
{
	switch (m_ePL_STATE)
	{
	case CCody::PL_IDLE:
...
void CCody::Move(const _float& fTimeDelta, _float3* vResultPos, _float3* vResultDir, _float fRotSpeed)
{
	_float3 vPos, vDir;
	_float fCosTh;
	_int iRotDir;
...
void CCody::Dash(const _float & fTimeDelta, _float fTime, _float fDashRatio)
{
...
void CCody::Dash(const _float & fTimeDelta, _float fDashRatio)
{
	_float3 vLook;
...
_bool CCody::Gravity(const _float & fTimeDelta)
{
	if (!m_bSimulate)
		return false;
	...
	else if (false == m_bStopGravity)
		m_fGravityTime += fTimeDelta;

	return false;
}

void CCody::Event(const _float & fTimeDelta)
{
...
void CCody::Set_Late_State(const _float & fTimeDelta)
{
	switch (m_ePL_STATE)
	{
...
void CCody::Late_Push_State(const _float & fTimeDelta)
{
	switch (m_ePickUpType)
	{
	case CPlayerValue::DUMBBELL:
		if (m_pPickUpTarget->Is<CDumbbell>()->Get_Move())
		{
			...
			if (m_bDumbbell)
			{
				Engine::StopNPlaySound(L"VF3_Dumbell_Cody.ogg", CSoundMgr::CHANNELID::Crusher, 0.3f);
				m_bDumbbell = false;
			}
		}
		break;
...
_bool CCody::JumpTo(_float fJumpPower, _float fTime, _vec3 vGoalPos, _float fTimeDelta)
{
...
void CCody::Shoot(const _float& fTimeDelta)
{
	if (!m_bShoot)
		return;
...
_bool CCody::MoveTo(const _float& fTimeDelta, _float fGoalTime, _vec3 vGoalPos, _bool bExceptY, _bool bSimilar, _float fDis)
{
...
_bool CCody::RotTo(const _float & fTimeDelta, _float fGoalTime,/* _vec3 vTargetLook, */_vec3 vGoalLook, _bool bSimilar/*, _float fRad*/)
{
...
_bool CCody::RotTo(_vec3 vGoalLook)
{
	_vec3 vLook;
	m_pTransformCom->Get_Info(INFO_LOOK, &vLook);
	D3DXVec3Normalize(&vLook, &vLook);
...
void CCody::Set_Pos(char * tagBoneName, const _float& fTimeDelta)
{
...
_bool CCody::Ground(const _float& fTimeDelta)
{
...
void CCody::BoneIntoPosition(char * tagBoneName, _vec3 vTargetPosition)
{
	const D3DXFRAME_DERIVED* pBone = m_pMeshCom->Get_FrameByName(tagBoneName);
...
_bool CCody::BoneIntoPosition(char * tagBoneName, _vec3 vTargetPosition, _float fGoalTime, _float fTimeDelta)
{
...
_vec3 CCody::DirBetweenBones(char * tagBoneName1, char * tagBoneName2)
{
...
void CCody::InteractionMove(_vec3 vDir, const _float& fTimeDelta)
{
	_vec3 vPos;
	PxExtendedVec3 pxPos;
	m_pController->move(PxVec3(vDir.x, vDir.y, vDir.z), 0, fTimeDelta, m_ControllFilter);
...
void CCody::InteractionSetPos(_vec3 vPos, const _float& fTimeDelta)
{
	if (m_bPhys)
...
void CCody::Idle_State(const _float & fTimeDelta)
{
...
void CCody::Move_State(const _float & fTimeDelta)
{
	if (Ground(fTimeDelta))
	{
		m_ePL_STATE = PL_FALL;
...
void CCody::Jump_State(const _float & fTimeDelta)
{
	// 점프 내려찍기
...
void CCody::Sit_State(const _float & fTimeDelta)
{
	if (!(m_iKeyState & SIT))
	{
		m_fHalfHeight = m_fShapeRadius + (m_fShapeHeight * 0.5f);
...
void CCody::Run_State(const _float & fTimeDelta)
{
	if (m_iKeyState & DASH)
	{
		m_ePL_STATE = PL_DASH;
...
void CCody::Dash_State(const _float & fTimeDelta)
{
	m_bTest = true;
...
void CCody::Hang_State(const _float & fTimeDelta)
{
	if (!m_bHangStart)
	{
		m_bHangStart = true;
	...
	else if (m_iKeyState & INTERACTION2)
	{
		...
		m_pMeshCom->Set_Animationset(Cody_Fall, 1.0f, 0.01f);
		return;
	}
...
void CCody::Climbing_State(const _float & fTimeDelta)
{
...
void CCody::Slide_State(const _float& fTimeDelta)
{
...
void CCody::Ride_State(const _float& fTimeDelta)
{
	// 탈출조건
	...
	{
		m_pPickUpTarget->Is<CVacuum>()->Cody_ride();
		m_bBossRide = true;
	}
...
void CCody::Dead_State(const _float & fTimeDelta)
{
	m_bPhys = false;
...
void CCody::Lever_State(const _float & fTimeDelta)
{
		...
		else
		{
			...
			/*if (false == m_bLeverRot)
			{
				if (D3DXVec3Length(&vGoalDis) <= 0.1f)
					...
					m_bLeverRot = RotTo(fTimeDelta, 0.2f, vLook, vGoalLook);
			}
			else
			{
				...
				else
				{
					...
					m_bLeverArriveRot = RotTo(fTimeDelta, 0.15f, vLook, vGoalDir, true);
				}
			}*/
...
void CCody::PickUp_State(const _float & fTimeDelta)
{
...
void CCody::HangSwitch_State(const _float & fTimeDelta)
{
	if (m_bSuccess)
	{
		m_ePre_State = m_ePL_STATE;
...
void CCody::Set_Dir(_float3 * vDir, _int * iRotDir, _float * fCosTh)
{
...
void CCody::Float_state(const _float & fTimeDelta)
{
	m_bStopGravity = true;
...
void CCody::Fall_State(const _float & fTimeDelta)
{
	if (m_bGround)
	{
		if (m_iKeyState & (MOVE_DOWN | MOVE_UP | MOVE_LEFT | MOVE_RIGHT))
...
void CCody::Update_Time(const _float& fTimeDelta)
{
...
void CCody::Push_State(const _float & fTimeDelta)
{
	if (false == m_bMoveTo)
	{
		if (false == m_bRotTo)
		...
		{
		case CPlayerValue::DUMBBELL:
			m_vTargetPos = m_pPickUpTarget->Is<CDumbbell>()->Get_Target1();
			break;
		}
	...
	else if (m_iKeyState & MOVE_UP)
	{
		...
		{
		case DUMBBELL:
			m_pPickUpTarget->Is<CDumbbell>()->Move(fTimeDelta, CDumbbell::DUMBBELL_CODY);
			break;
...
void CCody::Handle_State(const _float & fTimeDelta)
{
	if (true == m_bTest)
	{
...
void CCody::Button_State(const _float & fTimeDelta)
{
	if (false == m_bTest)
	{
	...
	{
		CGameObject* pCamera = Engine::Get_GameObject(L"GameObject", L"CodyCamera");
		if (!pCamera)
			return;
		m_ePL_STATE = PL_IDLE;
...
void CCody::Shoot_State(const _float & fTimeDelta)
{
...
void CCody::Event_Saw_Shoot(const _float & fTimeDelta)
{
	...
	{
		m_bFlyStart = true;
		m_eCodyAnim = Cody_Grind_Start;
		m_vFlyPos = pSaw->Get_AttachCodyPos();
		pSaw->Set_Cody(true);
...
void CCody::Event_Saw_Sliding(const _float & fTimeDelta)
{
	m_bStopGravity = true;
	if (m_iKeyState & INTERACTION2)
	{
	...
	if (false == m_bSlidingStart)
	{
		...
		m_eCodyAnim = Cody_Slide_Start;
	}

	if (true == m_bSawJumpTrigger)
	{
	...
	_vec3 vDir = _vec3(56.63f, 7.565f, 27.72f) - _vec3(-9.6f, 47.55f, 27.72f);
	D3DXVec3Normalize(&vDir, &vDir);
	...
	else if (m_iKeyState & MOVE_RIGHT)
	{
		...
		vRight *= fTimeDelta * 2.0f;
		m_pController->move(PxVec3(vRight.x, vRight.y, vRight.z), 0, fTimeDelta, m_ControllFilter);
		m_eCodyAnim = Cody_Slide_Right;
	}
	else
		m_eCodyAnim = Cody_Slide;
}

void CCody::Hit_State(const _float & fTimeDelta)
{
	if (m_bHitEnd)
	{
		if (m_iKeyState & (MOVE_DOWN | MOVE_UP | MOVE_LEFT | MOVE_RIGHT))
...
void CCody::Late_Event_Saw_Shoot(const _float & fTimeDelta)
{
	CCircleSaw* pSaw = Engine::Get_GameObject(L"GameObject", m_tagHandleName.c_str())->Is<CCircleSaw>();
	if (nullptr == pSaw)
	{
	...
	if (true == m_bFlyMove)
	{
		_vec3 vPos = Engine::Get_GameObject(L"GameObject", m_tagHandleName.c_str())->Is<CCircleSaw>()->Get_AttachCodyPos();
		m_fSawPosZ = vPos.z;
		m_pController->setFootPosition(PxExtendedVec3(vPos.x, vPos.y, vPos.z));
		m_pTransformCom->Set_Pos(vPos);
		m_pTransformCom->Update_Component(fTimeDelta);
...
void CCody::Late_Climbing_State(const _float & fTimeDelta)
{
	if (true == m_bAttach)
	{
...
void CCody::RideObjet(const _float & fTimeDelta)
{
	if (true == m_bRideObj)
	{
		_vec3 vDir = *((_vec3*)m_pAttachTarget->On_Attach(this));
...
void CCody::Key_Reset()
{
	m_iKeyState = NULL;
}


void CCody::Create_UI(const _tchar * pLayerTag)
{
	...
	m_pFuseSign->Set_Image(L"CW_ImageUI");
	m_pFuseSign->Set_Size(_vec3{ 64.f, 64.f, 0.f });
	m_pFuseSign->Set_Pos(_vec3{ 0.f, -0.f, 0.f });
	m_pFuseSign->Set_RenderID(RENDER_FINALUI);
	m_pFuseSign->Set_PassIndex(2);
	m_pFuseSign->Set_TextureIndex(19);
	m_pFuseSign->Set_Visible(false);
	NULL_CHECK_RETURN(pGameObject, );
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"Cody_FuseSign", pGameObject), );
	m_pFuseSign->AddRef();
...
void CCody::Update_UI(const _float & fTimeDelta)
{
	HP_UI(fTimeDelta);
	Sign_UI(fTimeDelta);
...
void CCody::HP_UI(const _float & fTimeDelta)
{
	if (m_pHp)
...
void CCody::Sign_UI(const _float & fTimeDelta)
{
	if (!g_bChinema)
	{

		MaySign_UI(fTimeDelta);
		FuseSign_UI(fTimeDelta);
	}
}

void CCody::MaySign_UI(const _float & fTimeDelta)
{
	CMay* pMay = (CMay*)Engine::Get_GameObject(L"GameObject", L"May");
	CTransform* pMayTrans = (CTransform*)pMay->Get_Component(L"Com_Transform", ID_DYNAMIC);
...
void CCody::Set_Damage(_int iDamage)
{
...
void CCody::FuseSign_UI(const _float & fTimeDelta)
{
	...
	_vec3 vPos = { ((vFusePos.x + 0.5f) * WINCX * 0.5f) + WINCX * 0.5f, (vFusePos.y + 1.f) * WINCY * 0.5f - WINCY, vFusePos.z };
	...
	if (FinalPos.z <= 0.f && FinalPos.x <= 850.f && FinalPos.y >= -1080 && FinalPos.x >= 0.f && FinalPos.y >= 0.f)
		FinalPos.z = 0.f;

	if (FinalPos.z < 0.f || FinalPos.z > 1.f)
		FinalPos *= -1.f;
...
void CCody::NormalMap_Dead(const _float & fTimeDelta)
{
...
void CCody::BossMap_Dead(const _float & fTimeDelta)
{
...
void CCody::FadeIn(const _float & fTimeDelta)
{
	if (!m_bFadeIn)
		return;
...
void CCody::FadeOut(const _float & fTimeDelta)
{
	if (!m_bFadeOut)
		return;
